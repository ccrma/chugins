...
class Mesh2D_stk : public Instrmnt
{
 public:
  //! Class constructor, taking the x and y dimensions in samples.
  Mesh2D_stk( unsigned short nX, unsigned short nY );
...
inline StkFrames& Mesh2D_stk :: tick( StkFrames& frames, unsigned int channel )
{
  unsigned int nChannels = lastFrame_.channels();
#if defined(_STK_DEBUG_)
  if ( channel > frames.channels() - nChannels ) {
    oStream_ << "Mesh2D_stk::tick(): channel and StkFrames arguments are incompatible!";
