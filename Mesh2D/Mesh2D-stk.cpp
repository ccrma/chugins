...
Mesh2D_stk :: Mesh2D_stk( unsigned short nX, unsigned short nY )
{
  if ( nX == 0.0 || nY == 0.0 ) {
    oStream_ << "Mesh2D_stk::Mesh2D_stk: one or more argument is equal to zero!";
...
Mesh2D_stk :: ~Mesh2D_stk( void )
{
}

void Mesh2D_stk :: clear( void )
{
  this->clearMesh();
...
void Mesh2D_stk :: clearMesh( void )
{
  int x, y;
  for ( x=0; x<NXMAX-1; x++ ) {
    for ( y=0; y<NYMAX-1; y++ ) {
...
StkFloat Mesh2D_stk :: energy( void )
{
  ...
  StkFloat t;
  StkFloat e = 0;
  if ( counter_ & 1 ) { // Ready for Mesh2D_stk::tick1() to be called.
    for ( x=0; x<NX_; x++ ) {
      for ( y=0; y<NY_; y++ ) {
        t = vxp1_[x][y];
        e += t*t;
  ...
  else { // Ready for Mesh2D_stk::tick0() to be called.
    for ( x=0; x<NX_; x++ ) {
      for ( y=0; y<NY_; y++ ) {
        t = vxp_[x][y];
        e += t*t;
...
void Mesh2D_stk :: setNX( unsigned short lenX )
{
  if ( lenX < 2 ) {
    oStream_ << "Mesh2D_stk::setNX(" << lenX << "): Minimum length is 2!";
    handleError( StkError::WARNING ); return;
  }
  else if ( lenX > NXMAX ) {
    oStream_ << "Mesh2D_stk::setNX(" << lenX << "): Maximum length is " << NXMAX << '!';
    handleError( StkError::WARNING ); return;
  }
...
void Mesh2D_stk :: setNY( unsigned short lenY )
{
  if ( lenY < 2 ) {
    oStream_ << "Mesh2D_stk::setNY(" << lenY << "): Minimum length is 2!";
    handleError( StkError::WARNING ); return;
  }
  else if ( lenY > NYMAX ) {
    oStream_ << "Mesh2D_stk::setNY(" << lenY << "): Maximum length is " << NXMAX << '!';
    handleError( StkError::WARNING ); return;
  }
...
void Mesh2D_stk :: setDecay( StkFloat decayFactor )
{
  if ( decayFactor < 0.0 || decayFactor > 1.0 ) {
    oStream_ << "Mesh2D_stk::setDecay: decayFactor is out of range!";
    handleError( StkError::WARNING ); return;
  }
...
void Mesh2D_stk :: setInputPosition( StkFloat xFactor, StkFloat yFactor )
{
  if ( xFactor < 0.0 || xFactor > 1.0 ) {
    oStream_ << "Mesh2D_stk::setInputPosition xFactor value is out of range!";
    handleError( StkError::WARNING ); return;
  }

  if ( yFactor < 0.0 || yFactor > 1.0 ) {
    oStream_ << "Mesh2D_stk::setInputPosition yFactor value is out of range!";
    handleError( StkError::WARNING ); return;
  }
...
void Mesh2D_stk :: noteOn( StkFloat frequency, StkFloat amplitude )
{
  // Input at corner.
  if ( counter_ & 1 ) {
...
void Mesh2D_stk :: noteOff( StkFloat amplitude )
{
  return;
}

StkFloat Mesh2D_stk :: inputTick( StkFloat input )
{
  if ( counter_ & 1 ) {
...
StkFloat Mesh2D_stk :: tick( unsigned int )
{
...
StkFloat Mesh2D_stk :: tick0( void )
{
  int x, y;
  StkFloat outsamp = 0;
...
StkFloat Mesh2D_stk :: tick1( void )
{
  int x, y;
  StkFloat outsamp = 0;
...
void Mesh2D_stk :: controlChange( int number, StkFloat value )
{
#if defined(_STK_DEBUG_)
  if ( Stk::inRange( value, 0.0, 128.0 ) == false ) {
    oStream_ << "Mesh2D_stk::controlChange: value (" << value << ") is out of range!";
    handleError( StkError::WARNING ); return;
  }
#endif
  ...
  if ( number == 2 ) // 2
    ...
    oStream_ << "Mesh2D_stk::controlChange: undefined control number (" << number << ")!";
