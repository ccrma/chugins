...
void Plateau::getParameters()
{
    ...
    else if (params[FREEZE_PARAM].getValue() <= 0.5f && inputs[FREEZE_CV_INPUT].getVoltage() <= 0.5f && freezeButtonState)
    {
    ...
    if ((params[CLEAR_PARAM].getValue() > 0.5f ||
         inputs[CLEAR_CV_INPUT].getVoltage() > 0.5f) &&
        !clear && cleared)
    {
        cleared = false;
        clear = true;
    ...
    case 0:
        ...
        break;
    case 1:
        preDelayCVSens = preDelayLowSens;
    }
