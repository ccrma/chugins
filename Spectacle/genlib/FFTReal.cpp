...
:	_length (length)
,	_nbr_bits (int (floor (logf ((float)length) / logf (2) + 0.5)))
,	_bit_rev_lut (int (floor (logf ((float)length) / logf (2) + 0.5)))
,	_trigo_lut (int (floor (logf ((float)length) / logf (2) + 0.5)))
,	_sqrt2_2 (flt_t (sqrtf (2) * 0.5))
{
