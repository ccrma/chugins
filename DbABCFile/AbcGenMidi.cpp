...
AbcGenMidi::writetrack(int xtrack)
{
    ...
    case NOTES:
        ...
        this->wctx.mfile.writeMetaEvent(0L, 
            AbcMidiFile::text_event, 
            annotation, strlen(annotation));
        trackvoice = this->trackdescriptor[xtrack].voicenum;
        break;
    case WORDS:
        this->wctx.kspace = 0;
        this->wctx.noteson = 0;
    ...
    if(xtrack == 0) 
    {
        this->wctx.mfile.writeTempo(this->wctx.tempo);
    ...
    /* write notes */
    j = 0;
    if((this->initState->voicesused) && (trackvoice != 1)) 
        j = this->findvoice(j, trackvoice, xtrack);

    this->wctx.resetBar();
    this->wctx.saveRepeatState(j, state);
    pass = 1;
    slurring = 0;
    was_slurring = 0;
    expect_repeat = 0;
    while(j < this->initState->nfeatures) 
    {
        ...
        case Abc::NOTE:
            ...
            if(this->wctx.noteson) 
            {
                ...
                {
                    if((this->wctx.notecount > 1) && 
                      ((note_num * fd.denom) !=  (note_denom * fd.num)))
                    {
                        char msg[100];
                        snprintf(msg, 100,
                            "unequal notes in chord %d/%d versus %d/%d",
                            note_num, note_denom, fd.num, fd.denom);
                    ...
                    if(slurring && 
                      this->initState->featurelist[j+2].feature == Abc::SLUR_OFF) 
                        slurring = 0; /* [SS] 2011-11-30 */
                    if(this->wctx.trim && !slurring && !graceflag) 
                    {
                        tnote_num = note_num;
                        ...
                        {
                            AbcMusic::addFraction(&tnote_num, &tnote_denom,
                                -this->wctx.trim_num, this->wctx.trim_denom);
                        }
                        if(tnote_denom <= 0)
                        {
                        ...
                        this->wctx.addtoQ(tnote_num, tnote_denom, 
                            fd.pitch +  this->wctx.transpose + this->wctx.global_transpose,
                            this->wctx.channel, effecton, 
                            -this->wctx.totalnotedelay -1);
                    } 
                    else 
                    if(this->wctx.expand) 
                    ...
                    {
                        this->wctx.addtoQ(note_num, note_denom, 
                            fd.pitch + this->wctx.transpose + this->wctx.global_transpose,
                            this->wctx.channel, effecton, 
                            -this->wctx.totalnotedelay -1);
                    }
        ...
        case Abc::CHORDOFFEX:
            ...
            if(this->wctx.trim) 
            {
                if(AbcMusic::gtFraction(note_num, note_denom,
                                this->wctx.trim_num, this->wctx.trim_denom))
                {
                    AbcMusic::addFraction(&note_num,&note_denom,
                                -this->wctx.trim_num, this->wctx.trim_denom);
                }
            }
            break;
        ...
        case Abc::PART:
            in_varend = 0;
            j = this->partbreak(xtrack, trackvoice, j);
            if (parts == -1) 
            {
                char msg[1];
                msg[0] = (char) fd.pitch;
                this->wctx.mfile.writeMetaEvent(0L, AbcMidiFile::marker, msg, 1);
            };
            break;
        case Abc::VOICE:
        ...
        case Abc::TEXT:
            if(texton) 
            {
                ...
                this->wctx.mfile.writeMetaEvent(0L, 
                    AbcMidiFile::text_event, s.c_str(), s.size());
            };
            break;
        case Abc::TITLE:
        ...
        case Abc::REP_BAR:  /* :|  */
            ...
            if(pass == 1) 
            {
                ...
                else 
                {
                    ...
                    this->wctx.restoreRepeatState(&j, state);
                    slurring = 0;
                    was_slurring = 0;
                    expect_repeat = 0;
                }
    ...
    */
                ...
                if (ntracks != 1) 
                { 
                    this->wctx.mfile.writeMetaEvent(this->wctx.delta_time_track0, 
                        AbcMidiFile::set_tempo, data, 3);
                    this->wctx.tracklen += this->wctx.delta_time_track0;
                    this->wctx.delta_time_track0 = 0L;
                } 
                else 
                {
                    this->wctx.mfile.writeMetaEvent(
        ...
        case Abc::COPYRIGHT:
            if (xtrack == 0)
            {
                ...
                this->wctx.mfile.writeMetaEvent(
                    this->wctx.delta_time, AbcMidiFile::copyright_notice, 
                    str.c_str(), str.size());
            }
            break;
        case Abc::SETTRIM:
    ...
    if((expect_repeat)&&(pass==1) && !this->initState->silent) 
    {
...
AbcGenMidi::inlist(int place, int passno)
{
    ...
    /* printf("passno = %d\n", passno); */
    FeatureDesc &fd = this->initState->featurelist[place];
    if(fd.denom != 0) 
    {
...
{
    this->wctx.mfile.writeMetaEvent(this->wctx.delta_time, 
        AbcMidiFile::text_event, s, strlen(s));
    this->wctx.tracklen = this->wctx.tracklen + this->wctx.delta_time;
    this->wctx.delta_time = 0L;
}
...
{
    int j = 0;
    while((j<MAXCHANS) && (this->channel_in_use[j] != 0)) 
        j = j + 1;
    if(j >= MAXCHANS && !this->no_more_free_channels) 
    {
...
void 
AbcGenMidi::noteon(int n, FeatureDesc &fd)
{
    int vel = 0;
    if(this->beatmodel != 0)
...
AbcGenMidi::midi_noteon(long delta_time, int pitch, int chan, int vel, int pitchbend)
{
    ...
    else 
    {
            ...
            chan != 9) 
        {
            ...
            this->wctx.mfile.writeMidiEvent(delta_time, 
        ...
        data[1] = (char) vel;
        this->wctx.mfile.writeMidiEvent(delta_time, 
            AbcMidiFile::note_on, chan, data, 2);
    }
}
...
AbcGenMidi::midi_noteoff(long delta_time, int pitch, int chan)
{
    ...
    {
        this->wctx.mfile.writeMidiEvent(delta_time, 
            AbcMidiFile::note_off, chan, data, 2);
    }
}
...
AbcGenMidi::midi_re_tune(int channel) 
{
    ...
    /* indicate that we are applying RPN fine and gross tuning using
        ...
        control 101 0  
        control 100 1 */
    AbcMidiFile &mfile = this->wctx.mfile;
    data[0] = 101; /* RPN command */
    data[1] = 0;   /* type of command */
    mfile.writeMidiEvent(0, AbcMidiFile::control_change, channel, data, 2);
...
AbcGenMidi::fillvoice(int partno, int xtrack, int voice)
{
    ...
    {
        snprintf(msg, 100, "genmidi.c:fillvoice partlabel %d out of range",
            this->wctx.partlabel);
        this->wctx.error(msg);
        return;
    }
...
AbcGenMidi::dodeferred(char const *s, int noteson)
{
    ...
    if(strcmp(command, "control") == 0)
    {
        ...
        this->wctx.mfile.writeMidiEvent(0, AbcMidiFile::control_change, 
                                        chan, data, n);
    ...
    if(strcmp(command, "controlstring") == 0)
    {
        ...
        if(this->wctx.nlayers >= MAXLAYERS) 
    ...
    if(strcmp(command,"portamento") == 0)
    {
        ...
        data[1] = 127;
        /* turn portamento on */
        this->wctx.mfile.writeMidiEvent(0, 
    ...
    if(strcmp(command, "snt") == 0)
    {
        ...
        this->wctx.mfile.singleNoteTuningChange(midikey,  midipitch);
        done = 1;
    }
    else 
...
AbcGenMidi::configure_gchord()
{
    ...
    Chord &chord = this->initState->chords[this->wctx.chordnum];
...
AbcGenMidi::write_program(int p, int channel)
{
    ...
    {
        this->wctx.mfile.writeMidiEvent(this->wctx.delta_time,
            AbcMidiFile::program_chng, channel, data, 1);
    }
    this->wctx.tracklen += this->wctx.delta_time;
    this->wctx.delta_time = 0L;
}
...
{
    FeatureDesc &fd = this->initState->featurelist[n];
