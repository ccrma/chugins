...
// The LUT is just a global, and we'll need the init function to be called 
// before use.
...
int32_t Freqlut::s_lut[N_SAMPLES + 1];

void 
Freqlut::init(double sample_rate) 
{
    double y = (1LL << (24 + MAX_LOGFREQ_INT)) / sample_rate;
    double inc = pow(2, 1.0 / N_SAMPLES);
    for(int i = 0; i < N_SAMPLES + 1; i++) 
    {
        s_lut[i] = (int32_t)floor(y + 0.5);
        y *= inc;
    }
}
...
int32_t 
Freqlut::lookup(int32_t logfreq) 
{
    int ix = (logfreq & 0xffffff) >> SAMPLE_SHIFT;
    int32_t y0 = s_lut[ix];
    int32_t y1 = s_lut[ix + 1];
